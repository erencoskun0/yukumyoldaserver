...
{
    public class GetAllVehicleQueryHandler : IRequestHandler<GetAllVehicleQuery, List<GetAllVehicleQueryResult>>
    {
        private readonly YukumYoldaContext _context;

        public GetAllVehicleQueryHandler(YukumYoldaContext context)
        {
            _context = context;
        }

        public async Task<List<GetAllVehicleQueryResult>> Handle(GetAllVehicleQuery request, CancellationToken cancellationToken)
        {

            var values = await _context.UserVehicles.Include(uv => uv.Vehicle).Where(uv => uv.Vehicle.IsReady == true).Include(uv => uv.User).Include(uv => uv.Vehicle.VehicleType).Include(uv => uv.Vehicle.VehicleBody).AsNoTracking().ToListAsync();

            if (values.Count == 0)
                throw new Exception("Hiç araç bulunamadı.");

            return values.Select(x => new GetAllVehicleQueryResult
            {
                Id = x.VehicleId,
                Name = x.User.Name,
